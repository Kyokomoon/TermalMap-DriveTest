...
def render_tile(tile, tx_lat, tx_lon, max_radius_km, model, scenario, inf_nlos_type, azimuth_deg=0, tilt_deg=0):
    ...
    lats, lons = pixel_to_ll(x_coords, y_coords, tile_bounds)
    d2d = haversine_distance(tx_lat, tx_lon, lats, lons)

    # --- Горизонтальный угол (азимут) ---
    d_lon = lons - tx_lon
    d_lat = lats - tx_lat
    angle_to_pixel = (np.degrees(np.arctan2(d_lon, d_lat)) + 360) % 360
    relative_theta = (angle_to_pixel - azimuth_deg + 360) % 360
    relative_theta[relative_theta > 180] = 360 - relative_theta[relative_theta > 180]

    # --- Вертикальный угол (подъём) ---
    height_diff = HT - HR
    vertical_angle = np.degrees(np.arctan2(height_diff, d2d))

    # --- Усиление антенны с учётом диаграммы ---
    gains = np.vectorize(antenna_gain)(relative_theta, vertical_angle)

    # --- Потери и мощность ---
    path_losses = np.vectorize(get_path_loss)(d2d, FREQUENCY, HT, HR, model, scenario, inf_nlos_type)
    rx_powers = PTX_SUB + gains - CABLE_LOSS - path_losses

    # --- Отрисовка, если сигнал сильнее порога ---
    for px in range(256):
        for py in range(256):
            if rx_powers[px, py] >= RECEIVER_SENSITIVITY:
                pixels[px, py] = get_gradient_color(rx_powers[px, py], d2d[px, py], max_radius_km)
            else:
...
def process_tile(tile, tx_lat, tx_lon, max_radius_km, model, scenario, inf_nlos_type, azimuth_deg):
    img, z, x, y = render_tile(tile, tx_lat, tx_lon, max_radius_km, model, scenario, inf_nlos_type, azimuth_deg)
    save_tile(img, z, x, y)
...
def generate_tiles(tx_lat, tx_lon, radius_km, model, scenario, inf_nlos_type, azimuth_deg=0):
    ...
    end_time = time.time()
    print(f"Общее время отрисовки: {end_time - start_time:.2f} секунд")

if __name__ == "__main__":
    TX_LAT = 55.015970
    TX_LON = 82.942783
    AZIMUTH = 270  # градусов
